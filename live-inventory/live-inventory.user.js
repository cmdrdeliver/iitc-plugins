...
// @description    Show current in-game inventory. Requires CORE subscription. Fork of DanielOnDiordna's fork of EisFrei's plugin — falls back to the previously cached inventory when the Intel API returns an empty result, instead of wiping the cache.
// @id             liveInventory@cmdrdeliver
...
- when the Intel API returns an empty inventory (result === []),
  the previously cached inventory is kept instead of being overwritten.
- the dialog title bar shows "⚠ CACHED" while the in-memory inventory
  is the previously cached copy after such a failed refresh. The marker
  clears on the next successful (non-empty) refresh.
- the cached-fallback flag persists across page reloads, so the marker
  stays visible until you successfully refresh.
...
<line x1="15" y1="8" x2="17" y2="10" />
    ...
    self.menu = function(submenu) {
        ...
        if (submenu == 'Items') {
            ...
            var countsdisplay = inventoryarea.appendChild(document.createElement('div'));
            countsdisplay.className = self.id + 'counts';
            countsdisplay.style.margin = '4px 0';
            countsdisplay.style.fontWeight = 'bold';
            ...
            countsdisplay.textContent = counts.countkeys + ' keys / ' + counts.countportals + ' portals';
            ...
            buttons['About'] = function() { self.about(); };

            let author = container.appendChild(document.createElement('div'));
            author.className = self.id + 'author';
            author.textContent = self.titleDefault + ' version ' + self.version + ' by ' + self.author;
    ...
    function loadInventory(silent,retry) {
        try {
            ...
            if (localData.cachedFallback) {
                self.usingCachedFallback = true;
                self.title = self.titleCached;
            }
        } catch (e) {
    ...
    function refreshInventory(silent,retrysubscription,retryinventory) {
        ...
        getSubscription(
            function(data) {
                ...
                getInventory(
                    function(data) {
                        // Empty-result fallback: when the Intel API returns result === [],
                        // keep the previously cached inventory instead of overwriting it.
                        if (Array.isArray(data.result) && data.result.length === 0) {
                            console.warn(self.titleDefault + ' - Intel API returned an empty inventory; keeping previously cached data');
                            self.setCachedFallback(true);
                            // honour the 10-minute rate-limit window so we don't hammer the API
                            self.saveExpireValue(Date.now() + 10 * 60 * 1000);
                            self.updateMenu();
                            if (silent !== true)
                                alert("WARN - Intel returned an empty inventory.\nPreviously cached data kept.\nDialog title bar shows ⚠ CACHED until the next successful refresh.");
                            return;
                        }
                        console.log(self.titleDefault + " - Inventory data received");
                        self.storeInventoryData(data);
                        self.setCachedFallback(false);
    ...
    self.about = function() {
        let container = document.createElement('div');
        container.innerHTML =
            '<p><b>' + self.titleDefault + '</b> &mdash; version ' + self.version + '</p>' +
            '<p>This plugin is the work of several authors. Please credit them all:</p>' +
            '<ul>' +
                '<li><b>EisFrei</b> &mdash; original author.<br>' +
                    '<a target="_blank" rel="noopener" href="https://github.com/EisFrei/IngressLiveInventory">github.com/EisFrei/IngressLiveInventory</a></li>' +
                '<li><b>DanielOnDiordna</b> &mdash; fork adding menu buttons, refresh feedback, portals list key count, capsule view, capsule keys on portals, zoom keys, draw bookmarks.<br>' +
                    '<a target="_blank" rel="noopener" href="https://github.com/DanielOndiordna/IngressLiveInventory">github.com/DanielOndiordna/IngressLiveInventory</a></li>' +
                '<li><b>CmdrDeLiver</b> &mdash; this fork: keeps the previously cached inventory when the Intel API returns an empty result, surfaces a "⚠ CACHED" marker in the dialog title bar while the cached fallback is active, moves the key / portal counts from the title bar into the dialog body, and adds this About dialog.<br>' +
                    '<a target="_blank" rel="noopener" href="https://github.com/cmdrdeliver/iitc-plugins">github.com/cmdrdeliver/iitc-plugins</a></li>' +
            '</ul>' +
            '<p>Distributed under the original terms; see linked repositories.</p>';
        window.dialog({
            html: container,
            title: self.titleDefault + ' - About',
            id: self.id + '-about',
            width: 500
        });
    };

    self.usingCachedFallback = false;
    self.setCachedFallback = function(active) {
        self.usingCachedFallback = !!active;
        self.title = self.usingCachedFallback ? self.titleCached : self.titleDefault;
        const ls = {};
        try {
            const localData = JSON.parse(localStorage[KEY_SETTINGS]);
            if (localData) {
        ...
        ls.cachedFallback = self.usingCachedFallback;
        localStorage[KEY_SETTINGS] = JSON.stringify(ls);
    };
